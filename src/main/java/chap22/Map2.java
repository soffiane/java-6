...
public class Map2 {
    public static void main(String args[]) {
        ...
        String ch = m.get("f");       // String ch = (String)m.get("f") ; <-- avant JDK5.0
        System.out.println("valeur associee a f :                " + ch);

        // ensemble des valeurs (attention, ici Collection, pas Set)
        Collection<String> valeurs = m.values(); // Collection valeurs = m.values() ;  <-
        ...
        // modification de la valeur associee a la cl� x
        String old = m.put("x", "25");       // String old = (String)m.put("x", "25") ; <-
        if (old != null)
